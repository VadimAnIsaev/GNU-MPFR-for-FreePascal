{
    ...
    Cuten virsion.
    Only works with mpfr_t type variables.
...
procedure mp_randinit_default(out state: randstate_t); cdecl; external LIBGMP name '__gmp_randinit_default';
{ Initialize state with a linear congruential algorithm X = (aX + c) mod 2^m2exp }
procedure mp_randinit_lc_2exp(out state: randstate_t; var a: mpz_t; c, m2exp: valuint); cdecl; external LIBGMP name '__gmp_randinit_lc_2exp';
{ Initialize state for a linear congruential algorithm as per gmp_randinit_lc_2exp }
function mp_randinit_lc_2exp_size(out state: randstate_t; size: sizeuint): longint; cdecl; external LIBGMP name '__gmp_randinit_lc_2exp_size';
{ Initialize state for a Mersenne Twister algorithm }
procedure mp_randinit_mt(out state: randstate_t); cdecl; external LIBGMP name '__gmp_randinit_mt';
{ Initialize rop with a copy of the algorithm and state from op }
procedure mp_randinit_set(out rop: randstate_t; var op: randstate_t); cdecl; external LIBGMP name '__gmp_randinit_set';
{ Set an initial seed value into state }
procedure mp_randseed(var state: randstate_t; var seed: mpz_t); cdecl; external LIBGMP name '__gmp_randseed';
{ Set an initial seed value into state }
procedure mp_randseed_ui(var state: randstate_t; seed: valuint); cdecl; external LIBGMP name '__gmp_randseed_ui';
{ Free all memory occupied by state }
procedure mp_randclear(var state: randstate_t); cdecl; external LIBGMP name '__gmp_randclear';
